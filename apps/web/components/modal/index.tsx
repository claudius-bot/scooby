...
export default function Modal({
  ...
  return (
    <AnimatePresence>
      {showModal && (
        <>
          {isMobile && !options?.noLeaflet && <Leaflet setShow={setShowModal}>{children}</Leaflet>}
          {(!isMobile || options?.noLeaflet) && (
            <>
              <motion.div
                ...
                exit={{ scale: 0.95 }}
                onMouseDown={(e) => {
                  if (desktopModalRef.current === e.target && !options?.notDismissible) {
                    setShowModal(false);
                  }
                }}
              >
              ...
              <motion.div
                ...
                className={cn(
                  'fixed inset-0 z-[60] bg-gray-100 bg-opacity-10 backdrop-blur',
